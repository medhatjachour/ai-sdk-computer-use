...
// Windows fix: xdg-app-paths (used by @vercel/sandbox) requires HOME to be defined.
// This runs at module evaluation time — before any Sandbox call can crash.
if (!process.env.HOME) {
  process.env.HOME =
...
export const getDesktop = async (id?: string) => {
  // Validate required env vars before attempting any SDK calls
  if (!process.env.VERCEL_TOKEN && !process.env.VERCEL_OIDC_TOKEN) {
    throw new Error(
      "Missing VERCEL_TOKEN — add it to .env.local (get one at vercel.com/account/tokens)"
    );
  }
  if (!process.env.SANDBOX_SNAPSHOT_ID) {
    throw new Error(
      "Missing SANDBOX_SNAPSHOT_ID — add it to .env.local"
    );
  }
